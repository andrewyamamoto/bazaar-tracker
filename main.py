...
# version counters for game data per user; increment when a user's runs change
game_data_versions = {}

def mark_games_changed(user_id: int) -> None:
    """Increment the game data version for the specified user."""
    game_data_versions[user_id] = game_data_versions.get(user_id, 0) + 1
...
async def index(request: Request, season_id: str = None):
    ...
    @ui.refreshable
    async def list_of_games(page_number: int = 1, page_size: int = 8) -> None:
        ...
        user_inner = await get_current_user()
        if not user_inner:
        ...
        total_games = await models.Game.filter(player=user_inner.id, season=season_num).count()
        games = (
            await models.Game.filter(player=user_inner.id, season=season_num)
            .order_by('-played')
        ...
        with ui.column().classes('w-full'):
            ...
            for game in games:
                ...
                with ui.element('div').style(grid_style):
                    ...
                    ui.label(
                        "Perfect Game" if game.wins == 10 and game.finished == 10 else f"{game.wins}/{game.finished}"
                    ).classes(f'truncate {placement_color} text-center')
        ...
        games_for_chart = await models.Game.filter(player=user_inner.id, season=season_num).all()
        hero_options = ['Dooley', 'Mak', 'Pygmalien', 'Vanessa']
        placement_counts = {hero: [0] * 11 for hero in hero_options}

        for g in games_for_chart:
            hero = g.hero.lower().capitalize()
            if hero in placement_counts and 0 <= g.wins <= 10:
                placement_counts[hero][g.wins] += 1
        ...
        with ui.row().classes('w-full mb-4 gap-4 flex flex-wrap items-start'):
            with ui.card().classes('w-full sm:w-[48%] min-w-[300px]'):
                ...
                for g in games_for_chart:
                    ...
                    if hero not in placement_counts_by_category:
                        continue
                    if g.wins == 10 and g.finished == 10:
                        placement_counts_by_category[hero][4] += 1
                    elif 8 <= g.wins <= 10 and g.finished > 10:
                ...
                columns = [
                    ...
                    {"name": h, "label": h, "field": h, "align": "center"}
                ...
                for idx, category in enumerate(placement_categories):
                    ...
                    for h in hero_options:
                        row[h] = placement_counts_by_category[h][idx]
                    rows.append(row)
            ...
            with ui.card().classes('w-full sm:w-[48%] min-w-[300px]'):
                ...
                for idx, category in enumerate(placement_categories):
                    row = {"placement": category}
                    for h in hero_options:
                        row[h] = unranked_counts_by_category[h][idx]
    ...
    async def create() -> None:
        ...
        # notify this user's dashboard that their runs changed
        mark_games_changed(player.value.id)
        ranked.value = False
        hero.value = None
        wins.value = 0
        finished.value = 0
    ...
    # automatically refresh the dashboard when this user's runs change
    session_version = game_data_versions.get(user.id, 0)

    def refresh_if_needed():
        nonlocal session_version
        current_version = game_data_versions.get(user.id, 0)
